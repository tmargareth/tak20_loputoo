...
?>
  ...
  try {
        ...
        $sql = 'SELECT added from ilm_ds18_out_west ORDER BY added DESC LIMIT 1';
        ...
        $date = date("d.m.Y", strtotime($row['added']));
            $sqlWest = 'SELECT celsius from ilm_ds18_out_west where added = '.$date;
            $statement = $connection->prepare($sql); // valmistab sql-i ette
            $statement->execute(); // Käivita SQL lause koos muutujatega
            $result = $statement->fetch();
            $row = ($result);
...
<div class="container text-body text-center">
    <h1 id="index-text" class="font-weight-bold" style="margin-top: 15px; margin-bottom: 25px">Temperatuurid läänes <br><span id="selected_date"></span>
    </h1>
    <div class="row">
    <div class="column">
    <!-- Kalender -->
